...
Трапезников Михаил 205 группа

Дневник решений и гипотез решений соревнования на платформе kaggle.

[Ссылка](https://github.com/rw404/Kaggle_comp) на репозиторий гитхаб(в истории коммитов занесено, когда и какие добавления в дневник
и в код производились)

Код итогового решения лежит в этом же репозитории, но для удобства прикреплена
[ссылка](https://github.com/rw404/Kaggle_comp/blob/main/Submissions/kaggle_compet.ipynb).

Дополнительно прикреплен файл ноутбука в формате html.
...
Подбор параметров методом наименьших квадратов для 405 коэффициентов не закончился, поэтому
пришлось его остановить для ускорения алгоритма модели. Модель же для 5-ти коэффициентов
закончила обучение(длительность примерно 3 часа, причем на TPU google collab). Начальные значения
массы: __2.13643149 2.10502024 1.98572813 1.21240885__. Предположение, что руды от 1-ой до 4-ой имеют
такие начальные значения. А добавочная масса: __8.05196239847112__.

Ошибка на тренировочной выборке уменьшилось, хоть и незначительно, стала __5.233833599502356__,
что меньше предыдущей __5.8...__. Ошибка на kaggle составила __5.22__.

В ходе поиска оптимизации моделей наткнулся на модуль `numba` и использование конструкции
`@jit(nopython=True)`, которые значительно ускоряют работу программы. Как окзалось, данный
модуль и эта конструкция транслируют код питона в машинный, поэтому выполнение происходит быстро
даже без подключения CUDA и вычислений на GPU.

Оптимизирован код методов: замена циклов векторными вычислениями, добавлены указания на создание
машинного кода операций, возникли следующие проблемы при реализации:
- Так как используется 3-х мерный тензор, то при подсчете среднего по срезам тензора(матрица,
  соответствующая испытанию(в тренировочных - 10000 испытаний)), то нужен указатель `axis = 0`,
  чтобы вернулся вектор, но jit не поддерживает задание параметров библиотечных функций, поэтому
  пришлось написать 4 присваивания вектору по соответствующим векторам(то есть срез от полученной
  матрицы - среза тензора по нужному индексу - марке руды). Производится подбор параметров.
  
__День 20__
---

Закончен подбор начальных значений для гипотезы, но среди возможных значений обнаружены числа,
меньшие 0 порядка $10^3$, что указывает на то, что гипотеза не верна, так как подбор
коэффицицентов неуспешен. Установлены границы от 0 до 1000, запущен еще один подбор, чтобы точно
убеиться в несостоятельности гипотезы и реализовать вторую гипотезу(которую упоминал 2-3 дня
назад).

Кстати, из-за оптимизации кода и указателя на компиляцию пргораммы перед запуском,
коэффициенты, которые простоым способом не находились и за __24 часа__ работы, были определены за
__3-4
часа__.

После окончания подбора параметров значения коэффициентов стали очень большими(порядка
10^3), при дополнительном уточнении, что начальные значения находятся в диапазоне от 0 до 1000,
рузультат был близок к границе(1000), поэтому результирующий вектор оказался очень
маленьким(каждый элемент порядка $10^-23$), для уточнения о неотрицательных и ограниченных
начальных массах значения не изменились, что говорит о несостоятельности гипотезы.

Проверка новой гипотезы, когда в каждой руде некоторое содержание целевой руды(поэтому
параметр, соответствующий содержанию, не превосходит 1 и не меньше 0).

__Идея новой гипотезы:__ процесс обогащения, то есть отделение целевой руды от общей "смеси"
посредством премешивания, регулируется интенсивностью.

По идеи гипотезы видно, что методы сброса руды с ленты, добавления руды на ленту не нуждаются в
переработке, а методы перемешивания нуждаются. Глобальное перемешивание теперь после стремления к
равномерному включает пятый параметр - целевую руду; Так как этот метод теперь не считает целевую
руду, как "мнимую" величину, а считает, как ценный материал, отделяемый от мусора, то в
глобальном перемешивании масса последней руды вычисляется, как среднее имеющейся на ленте
ценной руды. В локальном же перемешивании масса ценной руды вычисляется, как взвешенная сумма
масс каждой марки руды с коэффициентами - значениями концентраций, нормированная
интенсивностью перемешивания. __Новое__ - уменьшение массы имеющейся руды после вычисления
отделенного колличества ценной руды от общей массы.

Запущен процесс подбора параметров.

Подбор параметров не завершен, поэтому результаты будут завтра.

__День 21__
----

Эксперимент поиска параметров происходил очень долго, поэтому решено проверять вторую гипотезу с
использованием google colab, пока считаются параметры первой. Во второй гипотезе исследуется
корректность операции сброса руды с ленты:
- первая гипотеза подразумевает, что лента сдвигается на нужное число сегментов вперед, затем
  возвращается.
- вторая гипотеза подразумевает иное - лента всегда двигается по кругу(гусеница),
  тогда возвращения после сброса руды не будет(то есть движение происходит в одном направлении).
  
Подбор параметров длился долго, поэтому принято допущение, что для проверки корректности
операции сдвига можно допустить равенство начальных масс на сегментах ленты(то есть, что вместо 404
параметров можно подберать всего 4). Второй проблемой оказалось то, что диапазоны значений
масс и концентраций могут принадлежать разным интервалам, но если в алгоритм построения модели(то
есть тот метод, результат работы которого и улучшается через МНК) добавить обработку такой
ситуации, то есть присвоение концентрации значения `min(1, concentration)`. Однако после таких
преобразований метод считает этот признак незначимым, поэтому такое проебразование
делать нет смысла. Тогда предложена следующая последовательность действий:
1. Подобрать начальные массы(главное, чтобы одинаковым способом), потом оставить этот
   параметр фиксированным до следующего подбора
2. Подобрать добавочную массу
3. Найти нужные концентрации
4. Оценить ошибку

По результатам проверки гипотезы были установлены начальные значения масс, как равномерное
распределение от 0 до 4:

`[1.1955960047970793, 1.1274044093573958, 1.6543435331293264, 0.2510549071342152]`

Добавочной массой удобно было выбрать значение 9. По результатам расчета для двух гипотез
получены значения ошибки, целевого вектора, концентраций:
- Первая гипотеза(прямое и обратное движение): 
  
  ```bash
  6.158781717467326
  
  [18.20741265 15.53848269 20.87467638 ... 17.37931398 24.6147753 16.53966167]

  [0.84833569 0.84826887 0.85205062 0.86200424]
  ```
  
- Вторая гипотеза(только прямое движение):

  ```bash
  6.090500405150938
  
  [15.54350785 13.85866719 19.7371434 ... 14.91000588 23.3977764 12.79376452]
  
  [0.83979392 0.83970481 0.83995887 0.84453193]
  ```

Как видно, ошибка меньше у гипотезы прямого движения, при том значения концентраций
оказались схожими. 

После этого предполагалось, что концентрации достоверны и начался подбор начальных масс.
Оптимальными оказались те же равномерно распределенные величины, что и были найдены раньше.
После подбора оптимального значения добавочной массы выяснилось, что лучшими значениями являются
числа от 9 до 11(в этом диапазоне).

Установлено, что корректной реализацией операции движения является прямой проход.

После этого начался новый подбор параметров, но при указании флага, что значения должны быть
неотрицательными, получено, что МНК возвращает неверные характеристики, в примере подбора были
возвращены только единицы, причем они не совпадают с затравочными значениями, поэтому(хоть
физически начальная масса не может быть отрицательной) предполагается, что все же значения меняются
от $-\infty$ до $+\infty$. Запущен корректный подбор.

Подбор привел к значению ошибки `18.94350344415221`. Ошибка равна мат.ожиданию целевого вектора,
что указывает на плохое значение коэффициентов. Но начальные значения масс указываются равными
1 и изредка 2, при том эти значения не ограничивались параметрами, поэтому такие значения могут являться настоящими. Поэтому проводится дополнительный
подбор начального значения массы: `43.42135717`, а так как значение добавочной массы целое, то оно
считается равным `43`. Проверка ошибки показывает уже значение:
`6.336770573592784`, что гораздо лучше предыдущего значения.

Теперь начинается последний подбор - значений концентрации: `[0.16544567 0.49924067 0.68212242 0.7612292]`

Ошибка оказалась равной `6.289626943044672`, что лучше предыдущего значения, при том коэффициенты
концентрации были тоже неограничены и были подобраны корректно, что указывает на состоятельность
гипотезы, но ошибка не уменьшается ниже 6.2, поэтому ее нельзя считать абсолютно
корректной. 

Ошибка на kaggle составила: `6.26376`.

Дополнительно таким же методом проверены предыдущие гипотезы:
- Гипотеза без перемешиваний:
  - Начальные массы выбраны, как результаты предыдущией гипотезы(где почти все были равны единицам):
    ```bash 
    Вывод почти единичной матрицы
    Ошибка: 5.808728703775862
    Добавочная масса: 10.52511123
    Ошибка после подбора добавочной массы: 5.778842856390461
    Ошибка на kaggle: 5.86157
    ```
- Гипотеза с концентрациями, но с движением "вперед и назад":
  ```bash
  Начальные значения быстро подобраны и равны тем же числам, что и до этого
  Ошибка: 18.94350344415221
  Добавочная масса: 60.77402932
  Ошибка после подбора добавочной массы: 6.760340601838754
  Значения концентраций: [0.33975347 0.50310893 0.72123826 0.77166882]
  Ошибка после подбора концентраций: 6.843598520371121
  Ошибка на kaggle: 6.89253
  ```
__День 22__
---
